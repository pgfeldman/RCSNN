...
class NetworkxGraphing:
    ...
    def draw(self, title: str, draw_legend: bool = True, font_size: int = 15, do_show: bool = True,
             scalar: float = 100):
        f = plt.figure()
    ...
    def print_edge_data(self, filename: str = None):
        ...
        for key in s_keys:
            n = nx.all_neighbors(self.G, key)
            for node in n:
                print("\t[{}][{}]: Weight = {} data = {}".format(key, node, self.G[key][node]['weight'],
                                                                 self.G[key][node]))
...
def country_example():
    ...
    ng.add_weighted_nodes("United States", 393212, "Canada", 150222,
                          {"long_text": "A random number: {}".format(random.random())})
    ng.add_weighted_nodes("United States", 393212, "Mexico", 75295,
                          {"long_text": "A random number: {}".format(random.random())})
    ng.add_weighted_nodes("United States", 393212, "Bahamas", 4179,
                          {"long_text": "A random number: {}".format(random.random())})
    ng.add_weighted_nodes("United States", 393212, "Dominican Republic", 43409,
                          {"long_text": "A random number: {}".format(random.random())})
    ng.add_weighted_nodes("United States", 393212, "Cuba", 96501,
                          {"long_text": "A random number: {}".format(random.random())})
    ng.add_weighted_nodes("United States", 393212, "Haiti", 36445,
                          {"long_text": "A random number: {}".format(random.random())})
    ng.add_weighted_nodes("United States", 393212, "Jamaica", 45069,
                          {"long_text": "A random number: {}".format(random.random())})
    ng.add_weighted_nodes("United States", 393212, "Cayman Islands", 22537,
                          {"long_text": "A random number: {}".format(random.random())})
    ng.add_weighted_nodes("United States", 393212, "Turks and Caicos Islands", 28163,
                          {"long_text": "A random number: {}".format(random.random())})
...
def hierarchy_example():
    random.seed(1)
    ng = NetworkxGraphing(name="test", creator="Phil")
    ng.add_connected_nodes("top", "mid_1")
    ng.add_connected_nodes("top", "mid_2")
    ng.add_connected_nodes("mid_1", "bot_1")
    ng.add_connected_nodes("mid_1", "bot_2")
    ng.add_connected_nodes("mid_2", "bot_3")
    ng.draw("hierarchy", draw_legend=False, do_show=False, scalar=100.0)
    plt.show()
